...
-- Procedures
--
CREATE DEFINER=`u640280979_iyfdashboard`@`127.0.0.1` PROCEDURE `GetAttendanceRatioByGroupAndMonth` (IN `p_group_name` VARCHAR(50), IN `p_month` INT, IN `p_year` INT)   BEGIN
...
CREATE DEFINER=`u640280979_iyfdashboard`@`127.0.0.1` PROCEDURE `progressReportGroupWise` (IN `p_sessionName` VARCHAR(50), IN `p_selectedYear` INT, IN `p_selectedMonth` INT, IN `p_facilitatorId` VARCHAR(50), IN `p_groupPrefix` VARCHAR(50))   BEGIN
...
CREATE DEFINER=`u640280979_iyfdashboard`@`127.0.0.1` PROCEDURE `studentClassReport` (IN `in_student_id` INT)   BEGIN
  SELECT 
